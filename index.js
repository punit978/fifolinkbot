...
bot.onText(/\/start/, (msg) => {
  ...
  const welcomeMessage = `Hello, ${username}!\n\n`
    + 'Welcome to the Fifolink Link Shortener Bot!\n'
    + 'You can use this bot to shorten URLs. This service is provided by Fifolink.com.\n\n'
    + 'To shorten a URL, just type or paste the URL directly in the chat, and the bot will provide you with the shortened URL.\n\n'
    + 'If you haven\'t set your Fifolink API token yet, use the command:\n/setapikey YOUR_FIFOLINK_API_TOKEN\n\n'
...
bot.onText(/\/setapikey (.+)/, (msg, match) => {
  ...
  const response = `Your FIFOLINK API token set successfully. Your token: ${userToken}`;
...
async function shortenUrlAndSend(chatId, Url) {
  // Retrieve the user's FIFOLINK API token from the database
  const arklinksToken = getUserToken(chatId);

  if (!arklinksToken) {
    bot.sendMessage(chatId, 'Please provide your FIFOLINK API token first. Use the command: /setapikey YOUR_FIFOLINK_API_TOKEN');
    return;
  }

  try {
    const apiUrl = `https://fifolink.com/api?api=${arklinksToken}&url=${Url}`;
...
// Function to save user's FIFOLINK API token to the database (Replit JSON database)
